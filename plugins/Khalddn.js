...
let handler = async (m, { conn: _conn, args, usedPrefix, command, isOwner }) => {
  ...
  if (!((args[0] && args[0] == 'plz') || (await global.conn).user.jid == _conn.user.jid)) {
        throw `"هذا الأمر يمكن استخدامه فقط من قبل الروبوت الرئيسي." wa.me/${global.conn.user.jid.split`@`[0]}?text=${usedPrefix + command}`
}
...
if (methodCode && !conn.authState.creds.registered) {
    ...
    setTimeout(async () => {
        ...
        parent.sendFile(m.chat, 'https://telegra.ph/file/f6f26c3ddf1c1c03a52a4.png', 'qrcode.png', `*الكود الخاص قم بإدخاله في المكان الخاص كما في الصورة \n \n *${codeBot}*\n\n\n > Khaled BOT`, m)
        rl.close();
    }, 3000);
}
  ...
  setTimeout(() => {
    ...
    const code = lastDisconnect?.error?.output?.statusCode || lastDisconnect?.error?.output?.payload?.statusCode;
     ...
     if (code !== DisconnectReason.connectionClosed){ 
        parent.sendMessage(conn.user.jid, {text : `🚩 "اتصال مفقود، يتم إعادة الاتصال"`}, { quoted: m }) //reconectar
    } else {
        parent.sendMessage(m.chat, {text : `🚩 يتم إعادة الإتصال`}, { quoted: m }) // session cerrada
    }
    }
    //----
    ...
    await parent.sendMessage(m.chat, {text : args[0] ? `🟢 ثم الإتصال بالسيرفر \n \n مرحبا بك في بوت Storeat البوت مدعوم بالكثير من الخصائص كا صناعة الصور وارسال التطبيقات والفيديوهات وكل مايخص المستخدم \n\n تابعني على انستجرام\n www.instagram.com/kh_sh932` : `🟢 جاري الإتصال بالبوت الرئيسي`}, { quoted: m })
    await sleep(5000)
    if (args[0]) return
                await parent.sendMessage(conn.user.jid, {text : `يتم تحميل البوت على هاذا الجهاز... `}, { quoted: m })
